...
const FilterByUser = (tickets) => {
  ...
  tickets.forEach(ticket => {
    ...
    // Skip tickets with missing or empty userId
    if (!userId) return;
...
const KanbanMainPage = () => {
  const [filterType, setFilterType] = useState(() => {
    return localStorage.getItem('filterType') || 'status'; // Default to 'status'
  });
  const [sortType, setSortType] = useState(() => {
    return localStorage.getItem('sortType') || 'priority'; // Default to 'priority'
  });
  const [filteredTickets, setFilteredTickets] = useState([]);
  const [tickets, setTickets] = useState([]);
  const [users, setUsers] = useState([]);

  // Fetch the API data once when the component mounts
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { tickets, users } = await APiData();
        setTickets(tickets); // Set tickets to state
  ...
  const applyFilters = () => {
    ...
    // Apply grouping based on the selected filter type
    switch (filterType) {
      case 'status':
